using System.Collections.Generic;
using System.Threading.Tasks;
using Newtonsoft.Json;
using Beanz.Core.Areas.HummanResourceManagement.Masters;
using Beanz.DTOs.Areas.HummanResourceManagement.Masters;
using Beanz.DTOs.Common;
using Beanz.DTOs.BeanzRoutes;
using Beanz.IBusiness.Areas.HummanResourceManagement.Masters;

namespace Beanz.Business.Areas.HummanResourceManagement.Masters
{
    public class LoanTypeBusiness : ILoanTypeBusiness
    {
        private readonly ILoanTypeRepository _repository;

        public LoanTypeBusiness(ILoanTypeRepository repository)
        {
            _repository = repository;
        }

        // ---------- Validation Helpers ----------
        private static BeanzResponseDTO Fail(string code, string message)
            => new BeanzResponseDTO { ErrorCode = code, ErrorMessage = message };

        private static BeanzResponseDTO ValidateContext(BeanzCommonDTO common)
        {
            if (common == null)
                return Fail("ERR001", "Request payload is required.");
            if (common.CompanyID <= 0)
                return Fail("ERR002", "CompanyID is required.");
            if (common.UserID <= 0)
                return Fail("ERR003", "UserID is required.");
            return null;
        }

        private static BeanzResponseDTO ValidateKey(BeanzCommonDTO common)
        {
            var ctx = ValidateContext(common);
            if (ctx != null) return ctx;
            if (common.PrimaryKeyID <= 0)
                return Fail("ERR010", "PrimaryKeyID is required.");
            return null;
        }

        // ---------- Methods ----------
        public Task<List<LoanTypeDTO>> GetLoanTypesAsync(BeanzCommonDTO common)
        {
            var ctx = ValidateContext(common);
            if (ctx != null) return Task.FromResult(new List<LoanTypeDTO>());
            return _repository.GetLoanTypesAsync(common);
        }

        public async Task<BeanzResponseDTO> SetLoanTypesAsync(BeanzCommonDTO common)
        {
            var ctx = ValidateContext(common);
            if (ctx != null) return ctx;

            if (string.IsNullOrWhiteSpace(common.Json))
                return Fail("ERR011", "Request data (Json) is required.");

            LoanTypeDTO dto;
            try
            {
                dto = JsonConvert.DeserializeObject<LoanTypeDTO>(common.Json);
            }
            catch
            {
                return Fail("ERR012", "Invalid request payload format.");
            }

            if (dto == null)
                return Fail("ERR012", "Invalid request payload.");
            if (string.IsNullOrWhiteSpace(dto.LoanTypeCode))
                return Fail("ERR013", "LoanType Code is required.");
            if (string.IsNullOrWhiteSpace(dto.LoanTypeName))
                return Fail("ERR014", "LoanType Name is required.");

            return await _repository.SetLoanTypesAsync(common);
        }

        public Task<BeanzResponseDTO> PostLoanTypesAsync(BeanzCommonDTO common)
        {
            var ctx = ValidateKey(common);
            if (ctx != null) return Task.FromResult(ctx);
            return _repository.PostLoanTypesAsync(common);
        }

        public Task<BeanzResponseDTO> DelLoanTypesAsync(BeanzCommonDTO common)
        {
            var ctx = ValidateKey(common);
            if (ctx != null) return Task.FromResult(ctx);
            return _repository.DelLoanTypesAsync(common);
        }

        public Task<LoanTypeViewModel> GetInfoLoanTypesAsync(BeanzCommonDTO common)
        {
            var ctx = ValidateContext(common);
            if (ctx != null) return Task.FromResult(new LoanTypeViewModel());
            return _repository.GetInfoLoanTypesAsync(common);
        }

        public Task<List<BeanzlookupDTO>> LookUpLoanTypesAsync(BeanzCommonDTO lookup)
            => _repository.LookUpLoanTypesAsync(lookup);

        public Task<LoanTypeViewModel> PrintLoanTypesAsync(BeanzCommonDTO common)
        {
            var ctx = ValidateKey(common);
            if (ctx != null) return Task.FromResult(new LoanTypeViewModel());
            return _repository.PrintLoanTypesAsync(common);
        }

        public Task<BeanzResponseDTO> ApproveLoanTypesAsync(BeanzCommonDTO common)
        {
            var ctx = ValidateKey(common);
            if (ctx != null) return Task.FromResult(ctx);
            return _repository.ApproveLoanTypesAsync(common);
        }
    }
}
