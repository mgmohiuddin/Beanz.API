...
namespace Beanz.Data.Services.Areas.HumanResourceManagement.Masters
{
    public class PositionRepository : IPositionRepository
    {
        ...
        public PositionRepository(IMapper mapper)
        {
            _mapper = mapper;
        }

        public async Task<List<PositionDTO>> GetPositionsAsync(BeanzCommonDTO common)
        {
            string _sql = "hrms.GetMS_Positions";
            SqlParameter[] _parameters =
            {
                ...
                new SqlParameter("@PositionID", SqlDbType.Int) { Value = common.PrimaryKeyID }
            };
            var data = await DAL.GetObjectListWithParametersAsync<PositionDTO>(_sql, _parameters);
            return data;
        }

        public async Task<BeanzResponseDTO> SetPositionsAsync(BeanzCommonDTO common)
        {
            string _sql = "hrms.SetMS_Positions";
            SqlParameter[] _parameters =
            {
                new SqlParameter("@CompanyID", SqlDbType.Int) { Value = common.CompanyID },
                new SqlParameter("@UserID", SqlDbType.Int) { Value = common.UserID },
        ...
        public async Task<BeanzResponseDTO> PostPositionsAsync(BeanzCommonDTO common)
        {
            string _sql = "hrms.PostMS_Positions";
            SqlParameter[] _parameters =
            {
                new SqlParameter("@CompanyID", SqlDbType.Int) { Value = common.CompanyID },
                new SqlParameter("@UserID", SqlDbType.Int) { Value = common.UserID },
        ...
        public async Task<BeanzResponseDTO> DelPositionsAsync(BeanzCommonDTO common)
        {
            string _sql = "hrms.DelMS_Positions";
            SqlParameter[] _parameters =
            {
                new SqlParameter("@CompanyID", SqlDbType.Int) { Value = common.CompanyID },
                new SqlParameter("@UserID", SqlDbType.Int) { Value = common.UserID },
        ...
        public async Task<List<BeanzlookupDTO>> LookUpPositionsAsync(BeanzCommonDTO lookup)
        {
            string _sql = "hrms.LookupMS_Positions";
            SqlParameter[] _parameters =
            {
                ...
                new SqlParameter("@PositionID", SqlDbType.Int) { Value = lookup.PrimaryKeyID }
            };
            var data = await DAL.GetObjectListWithParametersAsync<BeanzlookupDTO>(_sql, _parameters);
            return data;
        }

        public async Task<PositionViewModel> GetInfoPositionsAsync(BeanzCommonDTO common)
        {
            string _sql = "hrms.GetInfoMS_Positions";
            ...
            var _positionDTOList = new List<PositionDTO>();
            bool _isPositionsConsumed = false;
            using (SqlConnection _sqlConnection = new SqlConnection(Utilities.Configuration.ConnectionString))
            {
                using (var objRecord = await SqlMapper.QueryMultipleAsync(_sqlConnection, _sql, p, commandType: CommandType.StoredProcedure))
                {
                    while (!objRecord.IsConsumed)
                    {
                        ...
                        var positionData = objTemp.Where(c => c.TableName == "MS_Positions");
                        if (positionData.Count() > 0 && !_isPositionsConsumed)
                        {
                            _positionDTOList = _mapper.Map<List<PositionDTO>>(positionData);
                            _isPositionsConsumed = true;
                            continue;
                        }
                    }
                }
                var _positionDTO = _positionDTOList.Where(x => x.PositionID == common.PrimaryKeyID).FirstOrDefault();
                var result = new PositionViewModel
                {
                    Positions = _positionDTOList,
                    Position = _positionDTO
                };
                return result;
            }
        }

        public async Task<PositionViewModel> PrintPositionsAsync(BeanzCommonDTO common)
        {
            string _sql = "hrms.PrintMS_Positions";
            SqlParameter[] paramList =
            {
                new SqlParameter("@CompanyID", SqlDbType.Int) { Value = common.CompanyID },
                new SqlParameter("@UserID", SqlDbType.Int) { Value = common.UserID },
        ...
        public async Task<BeanzResponseDTO> ApprovePositionsAsync(BeanzCommonDTO common)
        {
            string _sql = "hrms.ApproveMS_Positions";
            SqlParameter[] _parameters =
            {
                new SqlParameter("@CompanyID", SqlDbType.Int) { Value = common.CompanyID },
                new SqlParameter("@UserID", SqlDbType.Int) { Value = common.UserID },
