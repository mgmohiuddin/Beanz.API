...
namespace Beanz.Data.Services.Areas.HummanResourceManagement.Masters
{
    public class AllowanceRepository : IAllowanceRepository
    {
        ...
        public AllowanceRepository(IMapper mapper)
        {
            _mapper = mapper;
        }
        ...
        public async Task<List<BeanzlookupDTO>> LookUpAllowancesAsync(BeanzCommonDTO lookup)
        {
            ...
            SqlParameter[] _parameters =
            {
                ...
                new SqlParameter("@AllowanceID", SqlDbType.Int) { Value = lookup.PrimaryKeyID }
            };
        ...
        public async Task<AllowanceViewModel> GetInfoAllowancesAsync(BeanzCommonDTO common)
        {
            ...
            var _allowanceDTOList = new List<AllowanceDTO>();
            bool _isAllowancesConsumed = false;
            using (SqlConnection _sqlConnection = new SqlConnection(Utilities.Configuration.ConnectionString))
            {
                using (var objRecord = await SqlMapper.QueryMultipleAsync(_sqlConnection, _sql, p, commandType: CommandType.StoredProcedure))
                {
                    while (!objRecord.IsConsumed)
                    {
                        ...
                        var allowanceData = objTemp.Where(c => c.TableName == "MS_Allowances");
                        if (allowanceData.Count() > 0 && !_isAllowancesConsumed)
                        {
                            _allowanceDTOList = _mapper.Map<List<AllowanceDTO>>(allowanceData);
                            _isAllowancesConsumed = true;
                            continue;
                        }
                    }
                }
                var _allowanceDTO = _allowanceDTOList.Where(x => x.AllowanceID == common.PrimaryKeyID).FirstOrDefault();
                var result = new AllowanceViewModel
                {
                    Allowances = _allowanceDTOList,
                    Allowance = _allowanceDTO
                };
                return result;
            }
        }
