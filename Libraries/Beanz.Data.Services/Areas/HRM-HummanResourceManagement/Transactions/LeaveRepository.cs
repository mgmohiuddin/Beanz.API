...
namespace Beanz.Data.Services.Areas.HummanResourceManagement.Transactions
{
    public class LeaveRepository : ILeaveRepository
    {
        ...
        public LeaveRepository(IMapper mapper)
        {
            _mapper = mapper;
        }

        public async Task<List<LeaveDTO>> GetLeavesAsync(BeanzCommonDTO common)
        {
            string _sql = "hrms.GetLeaves";
            SqlParameter[] _parameters =
            {
                ...
                new SqlParameter("@LeaveID", SqlDbType.Int) { Value = common.PrimaryKeyID }
            };
            var data = await DAL.GetObjectListWithParametersAsync<LeaveDTO>(_sql, _parameters);
            return data;
        }

        public async Task<BeanzResponseDTO> SetLeavesAsync(BeanzCommonDTO common)
        {
            string _sql = "hrms.SetLeaves";
            SqlParameter[] _parameters =
            {
                new SqlParameter("@CompanyID", SqlDbType.Int) { Value = common.CompanyID },
                new SqlParameter("@UserID", SqlDbType.Int) { Value = common.UserID },
        ...
        public async Task<BeanzResponseDTO> PostLeavesAsync(BeanzCommonDTO common)
        {
            string _sql = "hrms.PostLeaves";
            SqlParameter[] _parameters =
            {
                new SqlParameter("@CompanyID", SqlDbType.Int) { Value = common.CompanyID },
                new SqlParameter("@UserID", SqlDbType.Int) { Value = common.UserID },
        ...
        public async Task<BeanzResponseDTO> DelLeavesAsync(BeanzCommonDTO common)
        {
            string _sql = "hrms.DelLeaves";
            SqlParameter[] _parameters =
            {
                new SqlParameter("@CompanyID", SqlDbType.Int) { Value = common.CompanyID },
                new SqlParameter("@UserID", SqlDbType.Int) { Value = common.UserID },
        ...
        public async Task<List<BeanzlookupDTO>> LookUpLeavesAsync(BeanzCommonDTO lookup)
        {
            string _sql = "hrms.LookupLeaves";
            SqlParameter[] _parameters =
            {
                ...
                new SqlParameter("@LeaveID", SqlDbType.Int) { Value = lookup.PrimaryKeyID }
            };
            var data = await DAL.GetObjectListWithParametersAsync<BeanzlookupDTO>(_sql, _parameters);
            return data;
        }

        public async Task<LeaveViewModel> GetInfoLeavesAsync(BeanzCommonDTO common)
        {
            string _sql = "hrms.GetInfoLeaves";
            ...
            var _leaveDTOList = new List<LeaveDTO>();
            bool _isLeavesConsumed = false;
            using (SqlConnection _sqlConnection = new SqlConnection(Utilities.Configuration.ConnectionString))
            {
                using (var objRecord = await SqlMapper.QueryMultipleAsync(_sqlConnection, _sql, p, commandType: CommandType.StoredProcedure))
                {
                    while (!objRecord.IsConsumed)
                    {
                        ...
                        var leaveData = objTemp.Where(c => c.TableName == "Leaves");
                        if (leaveData.Count() > 0 && !_isLeavesConsumed)
                        {
                            _leaveDTOList = _mapper.Map<List<LeaveDTO>>(leaveData);
                            _isLeavesConsumed = true;
                            continue;
                        }
                    }
                }
                var _leaveDTO = _leaveDTOList.Where(x => x.LeaveID == common.PrimaryKeyID).FirstOrDefault();
                var result = new LeaveViewModel
                {
                    Leaves = _leaveDTOList,
                    Leave = _leaveDTO
                };
                return result;
            }
        ...
        public async Task<LeaveViewModel> PrintLeavesAsync(BeanzCommonDTO common)
        {
            string _sql = "hrms.PrintLeaves";
            SqlParameter[] paramList =
            {
                new SqlParameter("@CompanyID", SqlDbType.Int) { Value = common.CompanyID },
                new SqlParameter("@UserID", SqlDbType.Int) { Value = common.UserID },
        ...
        public async Task<BeanzResponseDTO> ApproveLeavesAsync(BeanzCommonDTO common)
        {
            string _sql = "hrms.ApproveLeaves";
            SqlParameter[] _parameters =
            {
                new SqlParameter("@CompanyID", SqlDbType.Int) { Value = common.CompanyID },
                new SqlParameter("@UserID", SqlDbType.Int) { Value = common.UserID },
