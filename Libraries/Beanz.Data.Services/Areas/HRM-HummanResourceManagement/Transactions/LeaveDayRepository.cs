...
namespace Beanz.Data.Services.Areas.HummanResourceManagement.Transactions
{
    public class LeaveDayRepository : ILeaveDayRepository
    {
        ...
        public LeaveDayRepository(IMapper mapper)
        {
            _mapper = mapper;
        }

        public async Task<List<LeaveDayDTO>> GetLeaveDaysAsync(BeanzCommonDTO common)
        {
            string _sql = "hrms.GetLeaveDays";
            SqlParameter[] _parameters =
            {
                ...
                new SqlParameter("@LeaveDayID", SqlDbType.Int) { Value = common.PrimaryKeyID }
            };
            var data = await DAL.GetObjectListWithParametersAsync<LeaveDayDTO>(_sql, _parameters);
            return data;
        }

        public async Task<BeanzResponseDTO> SetLeaveDaysAsync(BeanzCommonDTO common)
        {
            string _sql = "hrms.SetLeaveDays";
            SqlParameter[] _parameters =
            {
                new SqlParameter("@CompanyID", SqlDbType.Int) { Value = common.CompanyID },
                new SqlParameter("@UserID", SqlDbType.Int) { Value = common.UserID },
        ...
        public async Task<BeanzResponseDTO> PostLeaveDaysAsync(BeanzCommonDTO common)
        {
            string _sql = "hrms.PostLeaveDays";
            SqlParameter[] _parameters =
            {
                new SqlParameter("@CompanyID", SqlDbType.Int) { Value = common.CompanyID },
                new SqlParameter("@UserID", SqlDbType.Int) { Value = common.UserID },
        ...
        public async Task<BeanzResponseDTO> DelLeaveDaysAsync(BeanzCommonDTO common)
        {
            string _sql = "hrms.DelLeaveDays";
            SqlParameter[] _parameters =
            {
                new SqlParameter("@CompanyID", SqlDbType.Int) { Value = common.CompanyID },
                new SqlParameter("@UserID", SqlDbType.Int) { Value = common.UserID },
        ...
        public async Task<List<BeanzlookupDTO>> LookUpLeaveDaysAsync(BeanzCommonDTO lookup)
        {
            string _sql = "hrms.LookupLeaveDays";
            SqlParameter[] _parameters =
            {
                ...
                new SqlParameter("@LeaveDayID", SqlDbType.Int) { Value = lookup.PrimaryKeyID }
            };
            var data = await DAL.GetObjectListWithParametersAsync<BeanzlookupDTO>(_sql, _parameters);
            return data;
        }

        public async Task<LeaveDayViewModel> GetInfoLeaveDaysAsync(BeanzCommonDTO common)
        {
            string _sql = "hrms.GetInfoLeaveDays";
            ...
            var _leaveDayDTOList = new List<LeaveDayDTO>();
            bool _isLeaveDaysConsumed = false;
            using (SqlConnection _sqlConnection = new SqlConnection(Utilities.Configuration.ConnectionString))
            {
                using (var objRecord = await SqlMapper.QueryMultipleAsync(_sqlConnection, _sql, p, commandType: CommandType.StoredProcedure))
                {
                    while (!objRecord.IsConsumed)
                    {
                        ...
                        var leaveDayData = objTemp.Where(c => c.TableName == "LeaveDays");
                        if (leaveDayData.Count() > 0 && !_isLeaveDaysConsumed)
                        {
                            _leaveDayDTOList = _mapper.Map<List<LeaveDayDTO>>(leaveDayData);
                            _isLeaveDaysConsumed = true;
                            continue;
                        }
                    }
                }
                var _leaveDayDTO = _leaveDayDTOList.Where(x => x.LeaveDayID == common.PrimaryKeyID).FirstOrDefault();
                var result = new LeaveDayViewModel
                {
                    LeaveDays = _leaveDayDTOList,
                    LeaveDay = _leaveDayDTO
                };
                return result;
            }
        }

        public async Task<LeaveDayViewModel> PrintLeaveDaysAsync(BeanzCommonDTO common)
        {
            string _sql = "hrms.PrintLeaveDays";
            SqlParameter[] paramList =
            {
                new SqlParameter("@CompanyID", SqlDbType.Int) { Value = common.CompanyID },
                new SqlParameter("@UserID", SqlDbType.Int) { Value = common.UserID },
        ...
        public async Task<BeanzResponseDTO> ApproveLeaveDaysAsync(BeanzCommonDTO common)
        {
            string _sql = "hrms.ApproveLeaveDays";
            SqlParameter[] _parameters =
            {
                new SqlParameter("@CompanyID", SqlDbType.Int) { Value = common.CompanyID },
                new SqlParameter("@UserID", SqlDbType.Int) { Value = common.UserID },
