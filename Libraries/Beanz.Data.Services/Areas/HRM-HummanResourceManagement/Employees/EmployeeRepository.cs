...
namespace Beanz.Data.Services.Areas.HummanResourceManagement.Employees
{
    public class EmployeeRepository : IEmployeeRepository
    {
        ...
        public async Task<List<EmployeeDTO>> GetEmployeesAsync(BeanzCommonDTO common)
        {
            string _sql = "hrms.GetEmployees";
            SqlParameter[] _parameters =
            {
                ...
                new SqlParameter("@EmployeeID", SqlDbType.Int) { Value = common.PrimaryKeyID }
            };
            var data = await DAL.GetObjectListWithParametersAsync<EmployeeDTO>(_sql, _parameters);
            return data;
        }

        public async Task<BeanzResponseDTO> SetEmployeesAsync(BeanzCommonDTO common)
        {
            string _sql = "hrms.SetEmployees";
            SqlParameter[] _parameters =
            {
                new SqlParameter("@CompanyID", SqlDbType.Int) { Value = common.CompanyID },
                new SqlParameter("@UserID", SqlDbType.Int) { Value = common.UserID },
        ...
        public async Task<BeanzResponseDTO> PostEmployeesAsync(BeanzCommonDTO common)
        {
            string _sql = "hrms.PostEmployees";
            SqlParameter[] _parameters =
            {
                new SqlParameter("@CompanyID", SqlDbType.Int) { Value = common.CompanyID },
                new SqlParameter("@UserID", SqlDbType.Int) { Value = common.UserID },
        ...
        public async Task<BeanzResponseDTO> DelEmployeesAsync(BeanzCommonDTO common)
        {
            string _sql = "hrms.DelEmployees";
            SqlParameter[] _parameters =
            {
                new SqlParameter("@CompanyID", SqlDbType.Int) { Value = common.CompanyID },
                new SqlParameter("@UserID", SqlDbType.Int) { Value = common.UserID },
        ...
        public async Task<List<BeanzlookupDTO>> LookUpEmployeesAsync(BeanzCommonDTO lookup)
        {
            string _sql = "hrms.LookupEmployees";
            SqlParameter[] _parameters =
            {
                ...
                new SqlParameter("@EmployeeID", SqlDbType.Int) { Value = lookup.PrimaryKeyID }
            };
            var data = await DAL.GetObjectListWithParametersAsync<BeanzlookupDTO>(_sql, _parameters);
            return data;
        }

        public async Task<EmployeeViewModel> GetInfoEmployeesAsync(BeanzCommonDTO common)
        {
            string _sql = "hrms.GetInfoEmployees";
            ...
            var _employeeDTOList = new List<EmployeeDTO>();
            bool _isEmployeesConsumed = false;
            using (SqlConnection _sqlConnection = new SqlConnection(Utilities.Configuration.ConnectionString))
            {
                using (var objRecord = await SqlMapper.QueryMultipleAsync(_sqlConnection, _sql, p, commandType: CommandType.StoredProcedure))
                {
                    while (!objRecord.IsConsumed)
                    {
                        ...
                        var employeeData = objTemp.Where(c => c.TableName == "Employees");
                        if (employeeData.Count() > 0 && !_isEmployeesConsumed)
                        {
                            _employeeDTOList = _mapper.Map<List<EmployeeDTO>>(employeeData);
                            _isEmployeesConsumed = true;
                            continue;
                        }
                    }
                }
                var _employeeDTO = _employeeDTOList.Where(x => x.EmployeeID == common.PrimaryKeyID).FirstOrDefault();
                var result = new EmployeeViewModel
                {
                    Employees = _employeeDTOList,
        ...
        public async Task<EmployeeViewModel> PrintEmployeesAsync(BeanzCommonDTO common)
        {
            string _sql = "hrms.PrintEmployees";
            SqlParameter[] paramList =
            {
                new SqlParameter("@CompanyID", SqlDbType.Int) { Value = common.CompanyID },
                new SqlParameter("@UserID", SqlDbType.Int) { Value = common.UserID },
        ...
        public async Task<BeanzResponseDTO> ApproveEmployeesAsync(BeanzCommonDTO common)
        {
            string _sql = "hrms.ApproveEmployees";
            SqlParameter[] _parameters =
            {
                new SqlParameter("@CompanyID", SqlDbType.Int) { Value = common.CompanyID },
                new SqlParameter("@UserID", SqlDbType.Int) { Value = common.UserID },
