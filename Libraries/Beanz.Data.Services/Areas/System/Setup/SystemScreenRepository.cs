...
{
    public class SystemScreenRepository : ISystemScreenRepository
    {
        private readonly IMapper _mapper;

        public SystemScreenRepository(IMapper mapper)
        {
            _mapper = mapper;
        }

        public async Task<List<SystemScreenDTO>> GetSystemScreensAsync(BeanzCommonDTO common)
        {
            string _sql = "ses.GetSystemScreens";
        ...
        public async Task<BeanzResponseDTO> SetSystemScreensAsync(BeanzCommonDTO common)
        {
            string _sql = "ses.SetSystemScreens";
            SqlParameter[] _parameters =
            {
                new SqlParameter("@CompanyID", SqlDbType.Int) { Value = common.CompanyID },
                new SqlParameter("@UserID", SqlDbType.Int) { Value = common.UserID },
        ...
        public async Task<BeanzResponseDTO> PostSystemScreensAsync(BeanzCommonDTO common)
        {
            string _sql = "ses.PostSystemScreens";
            SqlParameter[] _parameters =
            {
                new SqlParameter("@CompanyID", SqlDbType.Int) { Value = common.CompanyID },
                new SqlParameter("@UserID", SqlDbType.Int) { Value = common.UserID },
        ...
        public async Task<BeanzResponseDTO> DelSystemScreensAsync(BeanzCommonDTO common)
        {
            string _sql = "ses.DelSystemScreens";
            SqlParameter[] _parameters =
            {
                new SqlParameter("@CompanyID", SqlDbType.Int) { Value = common.CompanyID },
                new SqlParameter("@UserID", SqlDbType.Int) { Value = common.UserID },
        ...
        public async Task<List<BeanzlookupDTO>> LookUpSystemScreensAsync(BeanzCommonDTO lookup)
        {
            string _sql = "ses.LookupSystemScreens";
            SqlParameter[] _parameters =
            {
        ...
        public async Task<SystemScreenViewModel> GetInfoSystemScreensAsync(BeanzCommonDTO common)
        {
            string _sql = "ses.GetInfoSystemScreens";
            ...
            var _systemScreenDTOList = new List<SystemScreenDTO>();
            bool _isSystemScreensConsumed = false;
            using (SqlConnection _sqlConnection = new SqlConnection(Utilities.Configuration.ConnectionString))
            {
                using (var objRecord = await SqlMapper.QueryMultipleAsync(_sqlConnection, _sql, p, commandType: CommandType.StoredProcedure))
                {
                    while (!objRecord.IsConsumed)
                    {
                        ...
                        var systemScreenData = objTemp.Where(c => c.TableName == "SystemScreens");
                        if (systemScreenData.Count() > 0 && !_isSystemScreensConsumed)
                        {
                            _systemScreenDTOList = _mapper.Map<List<SystemScreenDTO>>(systemScreenData);
                            _isSystemScreensConsumed = true;
                            continue;
                        }
                    }
                }
                var _systemScreenDTO = _systemScreenDTOList.Where(x => x.SystemScreenID == common.PrimaryKeyID).FirstOrDefault();
                var result = new SystemScreenViewModel
                {
                    SystemScreens = _systemScreenDTOList,
                    SystemScreen = _systemScreenDTO
                };
                return result;
            }
        }

        public async Task<SystemScreenViewModel> PrintSystemScreensAsync(BeanzCommonDTO common)
        {
            string _sql = "ses.PrintSystemScreens";
            SqlParameter[] paramList =
            {
                new SqlParameter("@CompanyID", SqlDbType.Int) { Value = common.CompanyID },
                new SqlParameter("@UserID", SqlDbType.Int) { Value = common.UserID },
        ...
        public async Task<BeanzResponseDTO> ApproveSystemScreensAsync(BeanzCommonDTO common)
        {
            string _sql = "ses.ApproveSystemScreens";
            SqlParameter[] _parameters =
            {
                new SqlParameter("@CompanyID", SqlDbType.Int) { Value = common.CompanyID },
                new SqlParameter("@UserID", SqlDbType.Int) { Value = common.UserID },
